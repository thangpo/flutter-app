...
class AndroidConfigChecker:
    def __init__(self) -> None:
        ...
        # Dự án này dùng Gradle Kotlin DSL (.kts)
        self._project_gradle_path = "./android/build.gradle.kts"
        self._settings_gradle_path = "./android/settings.gradle.kts"
    ...
    def is_project_gradle_correct(self):
        # Với Kotlin DSL, plugin được khai báo ở settings.gradle.kts
        paths = [self._project_gradle_path, self._settings_gradle_path]
        for p in paths:
            try:
    ...
    def is_app_gradle_firebase_messaging_dependencies_correct(self):
        with open(os.path.abspath(self._app_gradle_path)) as file:
            content = file.read()
            # chấp nhận cả dạng không version (khi dùng BOM)
